/* Multiple Spin Pool Exchange (ME) Bloch Equation Kernel running at CUDA GPU */

#ifndef _BLOCH_KERNEL_ME_GPU_H_
#define _BLOCH_KERNEL_ME_GPU_H_

__global__ void
BlochKernelMEGPU(float Gyro, double *d_CS, float *d_Rho, float *d_T1, float *d_T2, float *d_K, float *d_Mz, float *d_My, float *d_Mx, float *d_Buffer,
		...
		for (int k=0; k < SpinMxZ; k++){
			...
			for (int q=0; q< SeqLen; q++){
				...
				for (int t=0; t < TypeNum; t++){
					...
					/* exchange, assume K = 0  for self pool exchange (i.e. no exchange) */
					ExpdtKout  = 1;
					ExpdtKxin  = 0;
					ExpdtKyin  = 0;
					ExpdtKzin  = 0;
					for (int p=0; p < TypeNum; p++){
						/* go away */
						p_d_K	  = d_K   +  k * SpinMxNum + tid   + (t * TypeNum + p) * SpinMxAllNum; 
						ExpdtKout *= exp(-(*p_d_dt) * (*p_d_K));
						
						/* come in */
						p_d_K	  = d_K   +  k * SpinMxNum + tid   + (p * TypeNum + t) * SpinMxAllNum; 
