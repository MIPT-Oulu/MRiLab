...
% MRiLab Version 1.2                           %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [rfAmp,rfPhase,rfFreq,rfCoil,GzAmp,GyAmp,GxAmp,ADC,Ext,uts,ts,flags]=PSD_T2Prep
global VCtl
global VVar
CV1=1.5e-3;
CV10=0;
...
tS=-VCtl.T2Prep;
if isempty(tS) | isempty(tE) | (tS>=tE)
error('SE setting is incorrect for Pulses 2!');
end
if VVar.TRCount<TRStart | VVar.TRCount>TREnd | mod(VVar.TRCount-TRStart,Freq)~=0 | strcmp(Switch,'off')
% do nothing
else
...
p.rfFreq=0;
p.rfPhase=0;
p.tEnd=VCtl.T2Prep-0.3e-3;
p.tStart=VCtl.T2Prep-0.5e-3;
if strcmp(p.Switch,'on')
if strcmp(p.AnchorTE,'on')
switch VCtl.TEAnchor
case 'Start'
